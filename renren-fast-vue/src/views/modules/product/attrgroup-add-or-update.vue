...
<script>
  export default {
    data () {
      return {
        ...
        dataRule: {
          ...
          catelogId: [
            { required: true, message: '所属分类id不能为空' }
          ]
        }
      }
    },
    methods: {
      ...
      handleChange(){
        console.log("handleChange",this.catIds)
        this.dataForm.catelogId = this.catIds[this.catIds.length-1]
      },
      getCategories() {
        this.$http({
          ...
          if (this.dataForm.attrGroupId) {
            this.$http({
              ...
              if (data && data.code === 0) {
                ...
                this.catIds = data.attrGroup.catelogPath
              }
            })
          }
        })
