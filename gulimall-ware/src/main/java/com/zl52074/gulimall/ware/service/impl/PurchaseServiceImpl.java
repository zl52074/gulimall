...
import com.zl52074.gulimall.common.constatnt.WareConstant;
...
import com.zl52074.gulimall.ware.vo.PurchaseItemDoneVo;
...
import com.zl52074.gulimall.ware.dao.PurchaseDao;
...
public class PurchaseServiceImpl extends ServiceImpl<PurchaseDao, PurchaseEntity> implements PurchaseService {
    ...
    public PageUtils queryPageUnreceived(Map<String, Object> params) {
        IPage<PurchaseEntity> page = this.page(
                new Query<PurchaseEntity>().getPage(params),
                new QueryWrapper<PurchaseEntity>().eq("status", WareConstant.PurchaseStatusEnum.CREATED)
                        .or().eq("status", WareConstant.PurchaseStatusEnum.ASSIGNED)
        );
        return new PageUtils(page);
    }
    ...
    public void mergePurchase(MergeVo mergeVo) {
        ...
        List<Long> detailsIds = mergeVo.getItems();

        //确认采购单状态是0,1才可以合并
        Collection<PurchaseDetailEntity> purchaseDetailEntities = purchaseDetailService.listByIds(detailsIds);
        ...
        List<PurchaseDetailEntity> collect = detailsIds.stream().map(detailId -> {
